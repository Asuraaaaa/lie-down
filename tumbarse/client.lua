...
function bedActive(x, y, z, heading)
    ...
    while not HasAnimDictLoaded('missfbi5ig_0') do
        Citizen.Wait(0)
    end --{dict='anim@gangops@morgue@table@', anim='ko_front'}
	...
	Citizen.CreateThread(function ()
	    ...
	    if (health < 200)  then
		TriggerEvent('esx:showNotification', '~g~ Te están realizando una prueba, espera a que vuelva el medico'); 
		Citizen.Wait(5000)
		...
		TriggerEvent('esx:showNotification', '~y~ Te están realizando una prueba, espera a que vuelva el medico'); 
	    end
	end)
